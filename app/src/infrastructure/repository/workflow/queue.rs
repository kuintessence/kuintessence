...
impl ReadOnlyRepository<Queue> for OrmRepo {
    async fn get_by_id(&self, uuid: Uuid) -> anyhow::Result<Queue> {
        let queue: Queue = queue::Entity::find_by_id(uuid)
...
impl MutableRepository<Queue> for OrmRepo {
    ...
    async fn insert(&self, entity: &Queue) -> anyhow::Result<Uuid> {
        ...
        let stmt = queue::Entity::insert(queue::ActiveModel {
            id: Set(entity2.id),
        ...
        .on_conflict(
            OnConflict::column(queue::Column::Id)
                .update_columns([
                    queue::Column::Memory,
                    queue::Column::CoreNumber,
                    queue::Column::StorageCapacity,
                    queue::Column::NodeCount,
                ])
                .to_owned(),
        )
