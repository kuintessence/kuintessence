...
use alice_architecture::{utils::*, IAggregateRoot};
...
#[derive(Debug, Clone, Default, IAggregateRoot)]
pub struct Queue {
    pub id: Uuid,
    pub name: String,
...
impl Queue {
    ...
    pub async fn cache_resource(queue: &Queue) -> Anyhow {
        let mut queue_id_to_cache_info = QUEUE_ID_TO_CACHE_INFO.lock().await;
    ...
    pub async fn task_started(queue: &Queue) -> Anyhow {
        let mut queue_id_to_cache_info = QUEUE_ID_TO_CACHE_INFO.lock().await;
        let old = queue_id_to_cache_info.entry(queue.id).or_insert(QueueCacheInfo::default());
        old.start_one(queue.max_running_task_count)
    }

    pub async fn is_resource_full(queue: &Queue) -> Anyhow {
    ...
    pub async fn get_cache_info(queue_id: Uuid) -> Anyhow<QueueCacheInfo> {
        let queue_id_to_cache_info = QUEUE_ID_TO_CACHE_INFO.lock().await;
...
impl QueueCacheInfo {
    pub fn is_full(&self, queue: &Queue) -> Anyhow {
    ...
    pub fn cache(&mut self, queue: &Queue) -> Anyhow {
    ...
    pub fn start_one(&mut self, max_running_task_count: Option<i64>) -> Anyhow {
