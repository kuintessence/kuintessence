...
use domain_content_repo::model::vo::abilities::software_computing::software::SoftwareSpec;
use serde::{Deserialize, Serialize};
use uuid::Uuid;

use crate::model::vo::{Requirements, ScriptInfo};
...
/// 任务执行完的状态
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "status", content = "queueId")]
...
#[cfg(test)]
mod tests {
    use super::{TaskResult, TaskResultStatus};
    use uuid::Uuid;
    ...
    fn asd() {
        let a = TaskResult {
            id: Uuid::new_v4(),
            status: TaskResultStatus::Start(Some(Uuid::new_v4())),
            message: "".to_string(),
            used_resources: None,
        };
        let b = serde_json::to_string(&a).unwrap();
        let c = r#"{"id":"397653f1-a471-427b-81b6-35b16157d224","status":"Start","message":"","used_resources":null}"#;
        let d = serde_json::from_str::<TaskResult>(c).unwrap();
        println!("{}", b);
        println!("{:?}", d);
    }
}
