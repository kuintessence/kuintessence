...
impl Package {
    ...
    pub fn extract_package(id: Uuid, bytes: &[u8]) -> anyhow::Result<Self> {
        ...
        // 找到清单文件
        let mut manifest_entry = tar_files
            .entries()?
            .find(|el| {
                let entry = el.as_ref().unwrap();
                let path = entry.path().unwrap();
                // 因为经过验证，只要是根路径下的 is_file 文件就一定是 manifest
                entry.header().entry_type().is_file()
                    && path.ancestors().count() == PACKAGE_ANCESTORS_COUNT
            })
            .ok_or(anyhow!("package: {id} has no manifest file!"))??;
        ...
        // 反序列化成清单对象
        let manifest_obj = serde_json::from_value::<Manifest>(serde_yaml::from_str::<
            serde_json::Value,
        >(&manifest_content)?)?;

        let data = match manifest_obj.ability {
            AbilityKind::SoftwareComputing(ref package) => match package {
                SoftwareComputingRepo::Software => Self::parse_software_data(tar_files_2)?,
                SoftwareComputingRepo::Usecase { .. } => Self::parse_usecase_data(tar_files_2)?,
            },
        };
