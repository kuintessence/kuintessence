...
test.each(changes)("%s clears fetched identity, history, reason and confirmation", async (change) => {
  const f = lifecycleFixture();
  vi.mocked(lifecycle.getSpackMaterialLifecycle).mockResolvedValueOnce(f.atRevision(1));
  const view = mount();
  inspectLifecycle(f.binding);
...
test.each(["success", "503", "invalid-response", "stop"])(
  "%s invalidates the selected ordinary detail and catalog without automatically repeating POST",
  async (outcome) => {
    const f = lifecycleFixture();
...
test.each(["success", "conflict", "uncertain then GET", "uncertain then edit"])(
  "%s protects a pending lifecycle write from catalog and import selection attempts",
  async (outcome) => {
    const f = lifecycleFixture();
...
test.each(["org/org-a/materials", "org/org-b/materials"])(
  "mobile policy permits only manageable namespace audit reads, never writes: %s",
  async (repository) => {
...
test.each(["management disabled", "reader", "non-member"])(
  "%s exposes no lifecycle operation despite stored admin authentication",
  async (mode) => {
