...
#[derive(Builder)]
pub struct NetDiskService {
    net_disk_repo: Arc<dyn INetDiskRepo + Send + Sync>,
    flow_draft_repo: Arc<dyn IReadOnlyRepository<WorkflowDraft> + Send + Sync>,
...
#[async_trait]
impl INetDiskService for NetDiskService {
...
impl NetDiskService {
    async fn get_root_id(&self, user_id: Option<Uuid>) -> AnyhowResult<Uuid> {
    ...
    async fn create_node_instance_file(
        ...
        let node_instance_dir_id = self
            .get_or_create_node_instance_dir(
                node_id,
                flow_instance_id,
                &node_name,
                &flow_name,
    ...
    async fn create_flow_draft_file(
        ...
        let parent_id = self.get_or_create_flow_draft_dir(flow_draft_id, &flow_name).await?;
    ...
    async fn create_normal_file(
        ...
        parent_id = Some(match parent_id {
            Some(el) => el,
            None => self.get_root_id(None).await?,
        });
    ...
    async fn get_or_create_flow_draft_dir(
        &self,
        flow_draft_id: Uuid,
        flow_name: &str,
    ) -> AnyhowResult<Uuid> {
        ...
        let flow_draft_root_dir_id = match self.net_disk_repo.get_flow_draft_root_id().await? {
            Some(el) => el,
            None => {
                let root_id = self.get_root_id(None).await?;
    ...
    async fn get_or_create_flow_instance_dir(
        ...
        let flow_instance_root_dir_id =
            match self.net_disk_repo.get_flow_instance_root_id(user_id).await? {
                Some(el) => el,
                None => {
                    let root_id = self.get_root_id(user_id).await?;
                    let id = self
                        .net_disk_repo
    ...
    async fn get_or_create_node_instance_dir(
        &self,
        node_id: Uuid,
        flow_instance_id: Uuid,
        node_name: &str,
        ...
        let flow_instance_dir_id =
            match self.net_disk_repo.get_node_instance_dir_id(node_id).await? {
                Some(el) => el,
                None => {
                    self.get_or_create_flow_instance_dir(flow_instance_id, flow_name, user_id)
                        .await?
                }
            };
        let node_instance_dir_id = self
