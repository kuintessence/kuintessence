...
use kernel::prelude::*;
use sea_orm::{
    prelude::Uuid, ColumnTrait, Condition, ConnectionTrait, EntityTrait, QueryFilter, QueryTrait,
};
use std::{str::FromStr, sync::atomic::Ordering};

#[async_trait]
impl IFileMetaRepo for SeaOrmDbRepository {
...
impl IReadOnlyRepository<FileMeta> for SeaOrmDbRepository {
    async fn get_by_id(&self, uuid: &str) -> anyhow::Result<FileMeta> {
        let model = FileMetadataEntity::find_by_id(Uuid::from_str(uuid)?)
            .one(self.db.get_connection())
            .await?
