...
#[derive(Builder)]
pub struct WorkflowScheduleService {
    ...
    download_service: Arc<dyn IStorageServerDownloadDispatcherService + Send + Sync>,
    usecase_select_service: Arc<dyn IUsecaseSelectService + Send + Sync>,
    text_storage_repository: Arc<dyn IDBRepository<TextStorage> + Send + Sync>,
}
...
impl IWorkflowScheduleService for WorkflowScheduleService {
    ...
    async fn schedule_next_nodes(&self, mode: ScheduleMode) -> anyhow::Result<()> {
        let (mut workflow_instance, entry_node_ids) = match mode {
            ScheduleMode::WorkflowInstanceId(id) => {
                ...
                self.workflow_instance_repository.update(workflow_instance.to_owned()).await?;
                ...
                for node_spec in node_specs.iter() {
                    if !entry_node_ids.contains(&node_spec.id) {
                        // 非入口节点状态更新为待命中
                        let mut stand_by_node_instance = self
                            .node_instance_repository
                            .get_by_id(&node_spec.id.to_string())
                            .await?;
                        ...
                        if !node_spec.batch_strategies.is_empty() {
                            ...
                            for el in stand_by_sub_node_instances.iter_mut() {
                                ...
                                // 更新批量子节点
                                self.node_instance_repository.update(el.to_owned()).await?;
                            }
                        }
                    }
                }
        ...
        for task_node_spec in task_node_specs.iter() {
            ...
            let mut node_instance =
                self.node_instance_repository.get_by_id(&task_node_spec.id.to_string()).await?;
    ...
    async fn pause_workflow(&self, id: Uuid) -> anyhow::Result<()> {
        ...
        for item in running_node_instances.iter_mut() {
            item.status = NodeInstanceStatus::Paused;
    ...
    async fn continue_workflow(&self, id: Uuid) -> anyhow::Result<()> {
        ...
        for item in paused_node_instances.iter_mut() {
            item.status = NodeInstanceStatus::Running;
            self.node_instance_repository.update(item.to_owned()).await?;
            self.usecase_select_service
                .operate_task(OperateTask {
                    operate: Operation {
    ...
    async fn terminate_workflow(&self, id: Uuid) -> anyhow::Result<()> {
        ...
        let mut running_node_instances = self
            ...
            .into_iter()
            .filter(|el| {
                if let NodeInstanceStatus::Running = el.status {
                    true
                } else {
                    matches!(el.status, NodeInstanceStatus::Paused)
                }
            })
            .collect::<Vec<_>>();
        for item in running_node_instances.iter_mut() {
...
mod tests {
    ...
    async fn load() -> Arc<WorkflowScheduleService> {
        let json_repository =
        ...
        text_storage_repository.expect_get_by_id().returning(|_| {
            Ok(TextStorage {
                key: Some(Uuid::from_str("0ec4e324-0d51-434e-baf5-36424a8e75f5").unwrap()),
                value: "5".to_string(),
            })
        });
    ...
    #[tokio::test]
    pub async fn test_first_schedule_next_nodes() {
        let workflow_schedule_service = load().await;
        let workflow_instance_id = workflow_schedule_service
        ...
        workflow_schedule_service
            .schedule_next_nodes(ScheduleMode::WorkflowInstanceId(workflow_instance_id))
            .await
            .unwrap()
    }

    #[tokio::test]
    pub async fn test_not_first_schedule_next_nodes() {
        let workflow_schedule_service = load().await;

        let finished_id = workflow_schedule_service
            .node_instance_repository
            .get_all()
            .await
            .unwrap()
            .iter()
            .filter(|el| el.status.eq(&NodeInstanceStatus::Finished))
            .last()
            .unwrap()
            .id
            .to_owned();
        ...
        workflow_schedule_service
            .schedule_next_nodes(ScheduleMode::NodeInstanceId(finished_id))
            .await
            .unwrap();
    }
}
