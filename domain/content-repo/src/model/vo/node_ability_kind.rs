...
use crate::model::{
    entity::Package,
    vo::abilities::software_computing::{
        software::materials::{inputs::FilesomeInput, outputs::FilesomeOutput},
        usecase::{CollectedOut, UsecaseSpec},
    },
};
    ...
    pub fn extract_packages(packages: Packages) -> anyhow::Result<Self> {
        match packages {
            Packages::SoftwareComputing(p1, p2) => {
                let (usecase_version_id, usecase_data) = p1.usecase_package_data().context(
                    format!("Package: {} is not usecase package.", p1.version_id),
                )?;
                let (software_version_id, software_data) = p2.software_package_data().context(
                    format!("Package: {} is not software packages.", p2.version_id),
                )?;
