use super::{ISubTaskService, ITaskReportService};
use crate::{
    models::{SubTask, Task, TaskDisplayType, TaskStatus},
...
pub trait ITaskSchedulerService {
    async fn enqueue_task(&self, task: &Task) -> anyhow::Result<()>;
    async fn schedule_next_task(&self) -> anyhow::Result<()>;
    async fn schedule_next_task_by_id(&self, id: uuid::Uuid) -> anyhow::Result<()>;
...
pub struct TaskSchedulerService {
    ...
    report_service: std::sync::Arc<dyn ITaskReportService + Sync + Send>,
    sub_task_services: HashMap<TaskDisplayType, std::sync::Arc<dyn ISubTaskService + Sync + Send>>,
    max_tasks_count: u32,
...
impl TaskSchedulerService {
    pub fn new(
        ...
        sub_task_services: HashMap<
            TaskDisplayType,
            std::sync::Arc<dyn ISubTaskService + Sync + Send>,
        >,
        max_tasks_count: u32,
    ) -> Self {
        Self {
...
#[async_trait::async_trait]
impl ITaskSchedulerService for TaskSchedulerService {
    async fn enqueue_task(&self, task: &Task) -> anyhow::Result<()> {
