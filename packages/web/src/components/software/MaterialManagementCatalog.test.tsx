...
function catalog(nextCursor: string | null = null): SpackMaterialManagementCatalog {
  return {
    releases: f.catalog.releases.map((release) => ({ ...release, state: "withdrawn", revision: 1 })),
    nextCursor,
  };
}
...
test.each(["", "public/materials?state=all", "org/org-b/private"])(
  "does not request invalid or forbidden repository %s, even on form submission",
  (name) => {
    mount();
...
test("uses opaque cursor history across empty filtered pages and refetches previous pages", async () => {
  ...
  expect(
    vi.mocked(client.listSpackMaterialManagement).mock.calls.map(([query]) => query),
  ).toEqual([
...
test.each(["repository", "state", "limit"])(
  "editing %s aborts pending reads, clears rows and cursors, and never searches automatically",
  async (field) => {
...
test.each([400, 401, 403, 422, 502, 503])(
  "HTTP %i on a cursor page clears results and retries from page one",
  async (status) => {
    vi.mocked(client.listSpackMaterialManagement)
      .mockResolvedValueOnce(catalog(token("z")))
...
test.each(["session", "repository", "locked", "edit", "refresh"])(
  "%s prevents selection of a previously rendered row",
  async (mode) => {
