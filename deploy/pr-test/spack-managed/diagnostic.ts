  ...
  async run(command, options) {
    ...
    for (const line of result.stderr.split("\n")) {
      if (
        ...
        /^ci-solved-node:package=(hello|gcc|gmake|glibc|compiler-wrapper|gcc-runtime|samtools|htslib|zlib|ncurses|bzip2|xz|pkgconf|pkg-config|diffutils|libiconv|python|perl|other) match=(unique|ambiguous|missing) hash=(true|false|unavailable) version=(true|false|unavailable) namespace=(true|false|unavailable) arch-profile=(true|false|unavailable) arch-lock=(true|false|unavailable) target-profile=(true|false|unavailable) target-lock=(true|false|unavailable) parameters=(true|false|unavailable) external=(true|false|unavailable) package-hash=(true|false|unavailable) dependencies=(true|false|unavailable)$/.test(line) ||
        /^ci-tree-entry:kind=(regular|directory|symlink|fifo|socket|other) nlink=([0-9]{1,5}|unavailable) basename=(tic|captoinfo|infotocap|other)$/.test(line) ||
