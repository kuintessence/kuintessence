use super::{
    IJobSchedulerService, ISoftwareDeployerService, ISubTaskReportService, ISubTaskService,
};
use crate::{
    models::{
        DeployerType, FacilityKind, FileStatus, FileTransferCommand, FileTransferStatus, FileType,
        ScriptInfo, TaskDisplayType, TaskStatus,
    },
...
pub trait IRunJobService: ISubTaskService {
...
#[async_trait::async_trait]
impl ISubTaskService for RunJobService {
    /// complete
    ...
    async fn refresh_status(&self, id: &str) -> anyhow::Result<()> {
        ...
        task.status = match job.state {
            ...
            | crate::models::JobState::Suspended
            | crate::models::JobState::Queuing
            | crate::models::JobState::Completing => TaskStatus::Running,
            crate::models::JobState::Completed => TaskStatus::Completing,
            crate::models::JobState::Failed | crate::models::JobState::Unknown => {
                TaskStatus::Failed
            }
        };
    ...
    async fn internal_run_job(&self, id: &str) -> anyhow::Result<()> {
        ...
        if can_run {
            ...
            match task.task_type.clone() {
                crate::models::TaskType::UsecaseExecution {
                    arguments,
                    environments,
                    std_in,
                    name,
...
#[async_trait::async_trait]
impl IRunJobService for RunJobService {
    async fn run_job(&self, id: &str) -> anyhow::Result<()> {
