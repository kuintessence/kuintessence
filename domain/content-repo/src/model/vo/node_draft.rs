...
use crate::model::vo::abilities::{
    ...
    software_computing::{
        ...
        usecase::{
            ...
            spec::{FileOutOrigin as ModelFileOutOrigin, InputSlot, OutputSlot, TextRule},
        },
    },
};
...
pub enum NodeDraftKind {
    ...
    SoftwareUsecaseComputing {
        ...
        /// 附加数据
        #[serde(default)]
        additional_data: Option<Requirements>,
    },
}
                ...
                let (file_name, is_batch) = match origin {
                    ModelFileOutOrigin::CollectedOut(collected_out_descriptor) => {
                        match &collected_outs
                            ...
                            .to
                        {
                            CollectTo::File(out_file) => match out_file {
                                OutFile::Normal(name) => (name.to_owned(), false),
                                OutFile::Batched(name) => (name.to_owned(), true),
                            },
                            CollectTo::Text => unreachable!(),
                        }
                    }
                    ModelFileOutOrigin::UsecaseOut(file_out_and_appointed_by) => {
                        match &filesome_outputs
                            ...
                            .file_kind
                        {
                            FileKind::Normal(name) => (name.to_owned(), false),
                            FileKind::Batched(name) => (name.to_owned(), true),
                        }
                    }
                };
        ...
        match l {
            InputSlot::Text {
                ...
                kind: NodeDraftInputSlotKind::Text {
                    contents: None,
                    rule: text_rule.map(TextInputSlotRule::from).unwrap_or_default(),
                },
                optional,
                descriptor,
                description: metadata.extra.get("description").map(|el| el.to_string()),
            ...
            InputSlot::File {
                ...
                let ref_file_descriptor = loop {
                    ...
                    if ref_file_descriptor.is_none() {
                        continue;
                    }
...
impl From<TextRule> for TextInputSlotRule {
    fn from(l: TextRule) -> Self {
        match l {
            TextRule::Json => Self::Json,
            TextRule::Number => Self::Number,
            TextRule::Regex(regex) => Self::Regex { regex },
        }
    }
}
